...
#OUTPUT=${wkd}AFR_${ANC}_sim_highcov_golden.fixed1.bam \
...
#$INPUT=${bam}
${EXE_JAVA} -Xmx2g -XX:ParallelGCThreads=1 -Djava.io.tmpdir=${tmp} -jar ${picard}AddOrReplaceReadGroups.jar \
    I= ${BAM}AFR/High/AFR_${ANC}_sim_reads_high_golden.calmd.bam \
    O= ${BAM}AFR/High/AFR_${ANC}_sim_reads_high_golden.calmd_gatk.bam \
